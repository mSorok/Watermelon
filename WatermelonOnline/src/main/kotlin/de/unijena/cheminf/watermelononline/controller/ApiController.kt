...
import de.unijena.cheminf.watermelononline.mongocollections.PubFingerprintsCounts
import de.unijena.cheminf.watermelononline.mongocollections.PubFingerprintsCountsRepository
import de.unijena.cheminf.watermelononline.mongocollections.WatermelonMolecule
import de.unijena.cheminf.watermelononline.mongocollections.WatermelonMoleculeRepository
import de.unijena.cheminf.watermelononline.utils.AtomContainerToUniqueNaturalProductService
...
@RestController
@RequestMapping("/api")
class ApiController(val watermelonMoleculeRepository: WatermelonMoleculeRepository) {
    /*
        ...
        try {
            ...
            when(ex) {
                is MongoCommandException, is OutOfMemoryError -> {
                    val other: List<WatermelonMolecule> = emptyList()
                    return mapOf(
    ...
    *  ************************************************************************************************
    ...
    fun doMassSearch(minMass:Double?, maxMass:Double?, maxHits:Int?): Map<String, Any>{
        ...
        if(minMass != null && maxMass != null){
            ...
            var results = this.watermelonMoleculeRepository.minMaxMolecularWeightSearch(minMass, maxMass, maxResults)
            ...
            println("only min")
            var results = this.watermelonMoleculeRepository.minMolecularWeightSearch(minMass, maxResults)
            println(results.size)
            ...
            println("only max")
            var results = this.watermelonMoleculeRepository.maxMolecularWeightSearch(maxMass, maxResults)
            println(results.size)
            ...
            var results = arrayListOf<WatermelonMolecule>()
...
/*
        ...
        val results = this.watermelonMoleculeRepository.findByChemclass(query)
...
*/
    ...
    fun doAdvancedSearch(maxHits:Int?, advancedSearchModel: AdvancedSearchModel) : Map<String, Any>{
        ...
        val results = this.watermelonMoleculeRepository.advancedSearchWithCriteria(advancedSearchModel, maxResults)
    ...
    fun doExactStructureSearchByInchi(smiles: String): Map<String, Any> {
        ...
        try {
            ...
            val results = this.watermelonMoleculeRepository.findByInchi(queryInchi)
    ...
    fun doExactStructureSearchBySmiles(smiles: String) : Map<String, Any>{
        ...
        try {
            ...
            val results = this.watermelonMoleculeRepository.findByUnique_Smiles(querySmiles)
    ...
    fun doSimpleSearchWithHeuristic(query: String): Map<String, Any> {
        ...
        val coconutPattern = Regex("^AFC[0-9]+?$")

        var naturalProducts : List<WatermelonMolecule>
        ...
        if(determinedInputType=="SMILES"){
            ...
            try {
                ...
                if(coconutPattern.containsMatchIn(query)) {
                    naturalProducts = this.watermelonMoleculeRepository.findByAfc_id(query)
                    determinedInputType = "AFC ID"
                }
                else if(inchiPattern.containsMatchIn(query)){
                    naturalProducts =  this.watermelonMoleculeRepository.findByInchi(query)
                    determinedInputType = "InChi"
                }
                else if(inchikeyPattern.containsMatchIn(query)){
                    naturalProducts =  this.watermelonMoleculeRepository.findByInchikey(query)
                    determinedInputType = "InChi Key"
                }
                else if(molecularFormulaPattern.containsMatchIn(query)){
                    naturalProducts = this.watermelonMoleculeRepository.findByMolecular_formula(query)
                    ...
                    //if (naturalProducts == null || naturalProducts.isEmpty()) {
                        ...
                        naturalProducts = this.watermelonMoleculeRepository.fuzzyNameSearch(altQuery)
                    //}
    ...
    fun doSubstructureSearch(smiles: String, type: String, maxHitsSubmitted: Int?): Map<String, Any> {
        ...
        val hits = mutableListOf<WatermelonMolecule>()
        var counter: Int = 0
        ...
        try {
            ...
            val matchedList = this.watermelonMoleculeRepository.findAllPubchemBitsSet(barray)
    ...
    fun doSimilaritySearch(smiles: String, maxHitsSubmitted: Int?, th: Int?): Map<String, Any> {
        ...
        try {
            ...
            val matchedList = this.watermelonMoleculeRepository.similaritySearch(requestedBits, queryPF, qmin, qmax, qLen, threshold, maxResults)
